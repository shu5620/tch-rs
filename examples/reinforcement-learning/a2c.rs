...
pub fn train() -> cpython::PyResult<()> {
    ...
    for update_index in 0..UPDATES {
        ...
        for s in 0..NSTEPS {
            ...
            let probs = actor.softmax(-1, Double);
            let actions = probs.multinomial(1, true).squeeze_dim(-1);
            let step = env.step(Vec::<i64>::from(&actions))?;

            sum_rewards += &step.reward;
            total_rewards += f64::from((&sum_rewards * &step.is_done).sum(Double));
            total_episodes += f64::from(step.is_done.sum(Double));
        ...
        let log_probs = actor.log_softmax(-1, Double);
        let probs = actor.softmax(-1, Double);
        ...
        let dist_entropy = (-log_probs * probs)
            .sum_dim_intlist(&[-1], false, Double)
            .mean(Double);
        let advantages = s_returns.narrow(0, 0, NSTEPS).to_device(device) - critic;
        let value_loss = (&advantages * &advantages).mean(Double);
        let action_loss = (-advantages.detach() * action_log_probs).mean(Double);
